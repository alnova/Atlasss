...
def gSearch(phrase, maxNumber,detailed=False, returnType='dictionary'):
    try:
        # query = phrase
        ...
        # print url
        # request = urllib2.Request(url, None, {'Referer': 'http://space-moon-111.appspot.com'})
        # print request
        ...
        response_data['id'] = []
        for h in hits:
            response_data['images'].append(h['url'])
            response_data['id'].append('')
...
def wolframAlphaSearch(word, maxNumber, detailed=False,returnType='dictionary'):
    """ Given a word, return the type of the word. """
    try:
        query = word
        from StringIO import StringIO
        io = StringIO()
        ...
        appid = 'WJYTHW-WVLY98YW77'
        input = 'define word '+ word
        wordTypeCount = {}
        result={}
        wordTypeList = ['noun','verb','adjective']
        wordTypeCount['noun'] = 0
        wordTypeCount['verb'] = 0
        wordTypeCount['adjective'] = 0
        # print 'ok'
        waeo = wap.WolframAlphaEngine(appid, server)
        query = waeo.CreateQuery(input)
        resultWQ = waeo.PerformQuery(query)
        #print result
        waeqr = wap.WolframAlphaQueryResult(resultWQ)
        # print waeqr
        jsonresult = waeqr.JsonResult()

        wordsOnPage = jsonresult.split()
        for each in wordsOnPage:
            for wordType in wordTypeList:
                if wordType in each:
                    wordTypeCount[wordType] += 1
        import operator
        result['word'] = word
        sorted_x = sorted(wordTypeCount.iteritems(), key=operator.itemgetter(1),reverse=True)
        result['type'] = sorted_x[0][0]
...
stopWordList = ['a', 'the', 'is', 'went', 'while', 'to', 'in','his', 'her']
